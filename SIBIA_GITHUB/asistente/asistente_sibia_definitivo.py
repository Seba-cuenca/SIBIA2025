...
class SistemaPersonalizacion:
    ...
    def __init__(self):
        self.archivo = "usuario_sibia.json"
...
class AsistenteSIBIADefinitivo:
    ...
    def _analizar_intencion(self, pregunta: str) -> Dict:
        ...
        patrones = {
            ...
            'calculo_energia': ['kw', 'energia', 'calcular', 'cuantos kw'],
...
{alertas_txt}"""
    ...
    def _calculo_energia(self, pregunta: str, ctx: Optional[ToolContext]) -> str:
        ...
        material = None
        if ctx.referencia_materiales:
            for mat in ctx.referencia_materiales.keys():
                if mat.lower() in pregunta:
                    material = mat
                    break
        
        match = re.search(r'(\d+(?:\.\d+)?)\s*(?:tn|ton)', pregunta)
        cantidad = float(match.group(1)) if match else None
...
"""
# En tu app.py de Flask:

from asistente_sibia_definitivo import asistente_sibia_definitivo, ToolContext
...
def chat():
    ...
    contexto = ToolContext(
        stock_materiales_actual=obtener_stock(),
        mezcla_diaria_calculada=obtener_mezcla(),
        referencia_materiales=REFERENCIA_MATERIALES,
        _calcular_kw_material_func=calcular_kw,
        _obtener_propiedades_material_func=obtener_props,
        _obtener_valor_sensor_func=leer_sensor
    )
...
@app.route('/api/sibia/stats', methods=['GET'])
def stats():
    return jsonify(asistente_sibia_definitivo.obtener_estadisticas())

@app.route('/api/sibia/voz', methods=['POST'])
def configurar_voz():
    data = request.get_json()
    habilitada = data.get('habilitada', True)
    return jsonify(asistente_sibia_definitivo.configurar_voz(habilitada))
